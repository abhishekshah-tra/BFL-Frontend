...
const CT_LAST_UPDATED = new Date('2025-05-20T10:30:00+04:00')
...
const WAREHOUSES: WarehouseId[] = ['YOTO', 'JAFZA', 'TECHNO']
...
const BN_NAV = [
  { page: 'process' as const, warehouse: 'TECHNO' as WarehouseId, process: 'Robo Sorting' as const },
  { page: 'operations' as const, warehouse: 'JAFZA' as WarehouseId },
  { page: 'operations' as const, warehouse: 'YOTO' as WarehouseId },
]

function parseChartPoints(pointsStr: string): { x: number; y: number }[] {
  return pointsStr
    .split(' ')
    .filter(Boolean)
    .map((pt) => {
...
function ControlTowerContent() {
  ...
  function fetchControlTowerData(key: TimeframeKey, opts: { silent?: boolean } = {}) {
    if (fetchTimer.current != null) window.clearTimeout(fetchTimer.current)
    setLoading(true)
  ...
  function handleTimeframeChange(label: string) {
  ...
  function handleRefresh() {
    ...
    showToast(
      `Dashboard refreshed – ${TIMEFRAME_LABELS[timeframe]} data updated as of ${new Date().toLocaleTimeString()}`,
    )
  }
  ...
  function handleKpiClick(label: string, detail: string, nav?: 'operations') {
    if (nav) {
      navigateTo(nav)
      return
    }
            ...
            {WAREHOUSES.map((id) => {
              ...
              return (
                <tr
                  ...
                  data-nav="operations"
                  data-warehouse={id}
                  data-process={id === 'TECHNO' ? 'Robo Sorting' : undefined}
                  id={`ct-wh-${id}`}
                  onClick={() =>
                    navigateTo('operations', {
                      warehouse: id,
                      ...(id === 'TECHNO' ? { process: 'Robo Sorting' as const } : {}),
                    })
                  }
                >
                  <td>
          ...
          {bottlenecks.map((bn, i) => {
            ...
            return (
              <div
                ...
                data-nav={nav.page}
                data-process={'process' in nav ? nav.process : undefined}
          ...
          >
            ...
            <g id="ct-chart-lines">
              ...
              <g data-series="#28a745">
                {yotoPts.map((p, i) => (
                  <circle key={`yoto-${i}`} cx={p.x} cy={p.y} r="3" fill="#28a745" />
                ))}
              </g>
              <g data-series="#0056b3">
                {jafzaPts.map((p, i) => (
                  <circle key={`jafza-${i}`} cx={p.x} cy={p.y} r="3" fill="#0056b3" />
                ))}
              </g>
              <g data-series="#d93025">
                {technoPts.map((p, i) => (
                  <circle key={`techno-${i}`} cx={p.x} cy={p.y} r="3" fill="#d93025" />
                ))}
              </g>
            </g>
