...
export type WarehouseId = 'TECHNO' | 'YOTO' | 'JAFZA';

export type TimeframeKey = 'today' | 'yesterday' | 'last7';

export type FlowStatus = 'good' | 'at-risk' | 'critical';

export type ProcessStatus = 'Good' | 'At Risk' | 'Critical';

export type StatusClass = 'good' | 'at-risk' | 'critical';

export type TrendClass = 'up' | 'down' | 'orange';

export type BarClass = 'green' | 'orange' | 'red';
...
export interface OpsKpis {
  ...
  throughputTrendClass: TrendClass;
  capacity: string;
  capacityTrend: string;
  capacityTrendClass: TrendClass;
  sla: string;
  slaTrend: string;
  slaTrendClass: TrendClass;
  wait: string;
  waitTrend: string;
  waitTrendClass: TrendClass;
  alerts: string;
...
export type OpsFlow = Record<ProcessName, FlowStatus>;

export interface OpsWarehouseData {
  kpis: OpsKpis;
  insight: OpsInsight;
  flow: OpsFlow;
  processes: OpsProcessRow[];
}

export type OpsTimeframeData = Record<
  TimeframeKey,
  Record<WarehouseId, OpsWarehouseData>
>;

export interface CtKpiMetric {
  value: string;
  unit?: string;
  trend?: string;
...
export interface CtWarehouseRow {
  throughput: string;
  capacity: number;
  barClass: BarClass;
  sla: string;
...
export interface CtTimeframeSlice {
  kpis: {
    throughput: CtKpiMetric;
    capacity: CtKpiMetric;
    sla: CtKpiMetric;
    bottlenecks: CtKpiMetric;
    alerts: CtKpiMetric;
  };
  warehouses: Record<WarehouseId, CtWarehouseRow>;
  bottlenecks: CtBottleneck[];
  chart: CtChart;
  recommendation: CtRecommendation;
}

export type CtTimeframeData = Record<TimeframeKey, CtTimeframeSlice>;

export const processData: Record<ProcessName, ProcessDetail> = {
  'Robo Sorting': {
    status: 'Critical',
    statusColor: '#dc2626',
    queue: '1,180 Units',
...
export const OPS_PROCESS_KEYS: ProcessName[] = [
  'Receive',
  'Checking',
  'Tagging',
  'Allocation',
...
export const OPS_TIMEFRAME_DATA: OpsTimeframeData = {
  today: {
    TECHNO: {
      kpis: {
        throughput: '51,300',
...
export const CT_TIMEFRAME_KEY: Record<string, TimeframeKey> = {
  Today: 'today',
  Yesterday: 'yesterday',
  'Last 7 Days': 'last7',
};

export const CT_TIMEFRAME_DATA: CtTimeframeData = {
  today: {
    kpis: {
      throughput: {
        value: '51,200',
